    ...
    if(isset($_POST['user-registration'])){
        ...
        if($result = $connect ->query($sel)){
            ...
            if( $generate_login != $login_in_dataBase && $generate_login != "" && $generate_pass == password_verify($generate_rep_pass, $generate_pass) && $generate_rep_pass !== ""
                ...
                "INSERT INTO registration (user_login, user_email, user_pass, data_registration, lock_user, unlock_user) 
                VALUES ('$generate_login', '$generate_email', '$generate_pass', '$data_registration_user', '$lock', '$unlock');";

                if($connect -> query($add_data_registration_user)){
                    echo "Пользователь зарегистрирован";
                    header('location: ../index.php?page=enter');
                }
                else{
    ...
    if(isset($_POST['user-autorisation'])){
        ...
        if($result = $connect->query($sel)){
            foreach($result as $row){
                ...
                $locked = $row['lock_user']; 
            }
            if($enter_login == $login_user_db && password_verify($enter_pass, $password_user_db) && $locked !=1){
                session_start();
                $_SESSION['login'] = $login_user_db;
                echo "вошел в систему" .$row['id'];
                header('refresh: 2, url = ../index.php?page=enter');
            } 
                ...
                echo "Пользователь " .$enter_login. " заблокирован";
                header('refresh: 3, url = ../index.php?page=enter');
            }
